...
def get_region_sides(region):
    ...
    for (x, y) in region:
        ...
        west = x, y - 1
        nw = x - 1, y - 1
        if not north in region:
            same_edge = (west in region) and (nw not in region)
            if not same_edge: 
                sides += 1
        ...
        if south not in region:
            same_edge = (west in region) and (sw not in region)
            if not same_edge: 
                sides += 1

        if west not in region:
            same_edge = (north in region) and (nw not in region)
            if not same_edge: 
                sides += 1
        ...
        if east not in region:
            same_edge = (north in region) and (ne not in region)
            if not same_edge: 
                sides += 1
