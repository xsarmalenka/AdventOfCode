...
def find_antinodes(positions):
    ...
    for ops in all_combinations:
        ...
        if first_ops != second_ops: 
            ...
            diff_row = first_ops_row - second_ops_row
            diff_clm = first_ops_clm - second_ops_clm

            while(first_ops_row > 0 and first_ops_clm > 0 and first_ops_row < limit and first_ops_clm < limit):
                antinode_row = first_ops_row + (diff_row)
                antinode_clm = first_ops_clm + (diff_clm)
                antinodes = add_antinode(antinodes, antinode_row, antinode_clm)
                first_ops_row = antinode_row
                first_ops_clm = antinode_clm

            while(second_ops_row > 0 and second_ops_clm > 0 and second_ops_row < limit and second_ops_clm < limit):
                antinode_row = second_ops_row - (diff_row)
                antinode_clm = second_ops_clm - (diff_clm)
                antinodes = add_antinode(antinodes, antinode_row, antinode_clm)
                second_ops_row = antinode_row
                second_ops_clm = antinode_clm
...
antenas_count = sum(1 for char in input if char.isalnum())
...
print("\nRESULT AFTER DUPLICITY REMOVER WITH ANTENAS")
print(result-dupliciter+antenas_count)
